package com.ddong.qingjie.batteryfragment;
...
public class BattRankInfoProvider {
    ...
    private final List<BattRankInfo> mUsageList = new ArrayList<>();
    private final List<BattRankInfo> mWifiSippers = new ArrayList<>();
    private final List<BattRankInfo> mBluetoothSippers = new ArrayList<>();
    ...
    public BattRankInfoProvider(Context context) {
        testType = 1;
        mContext = context;
     ...
     */
    public List<BattRankInfo> getBatteryStats() {
        ...
        final List<BattRankInfo> list = new ArrayList<>();

        Collections.sort(mUsageList);
        for (BattRankInfo sipper : mUsageList) {
    ...
    private List<BattRankInfo> getAppListCpuTime() {
        final List<BattRankInfo> list = new ArrayList<>();
        long totalTime = 0;
        ActivityManager am = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
        List<RunningAppProcessInfo> runningApps = am.getRunningAppProcesses();
        HashMap<String, BattRankInfo> templist = new HashMap<>();
        for (RunningAppProcessInfo info : runningApps) {
            ...
            if (pkgNames == null) {
                if (templist.containsKey(info.processName)) {
                    BattRankInfo sipper = templist.get(info.processName);
                    sipper.setValue(sipper.getValue() + time);
                    //YLog.e("tiem11", time + "");
                } else {
                    templist.put(info.processName,
                            new BattRankInfo(mContext, info.processName, time));
                }
                totalTime += time;
            } else {
                for (String pkgName : pkgNames) {
                    ...
                    if (templist.containsKey(pkgName)) {
                        sipper = templist.get(pkgName);
                        sipper.setPkgName(pkgName);
                        sipper.setValue(sipper.getValue() + time);
                    } else {
                        templist.put(pkgName, new BattRankInfo(mContext, pkgName, time));
                    }
                    totalTime += time;
                }
            }
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            BattRankInfo sipper = list.get(i);
            double percentOfTotal = sipper.getValue() * 100 / totalTime;
            if (percentOfTotal < mMinPercentOfTotal) {
                list.remove(i);
            } else {
                sipper.setPercent(percentOfTotal);
                //YLog.e("info_sipper", sipper.getValue() + "");
            }
        }

        Collections.sort(list, new Comparator<BattRankInfo>() {
            @Override
            public int compare(BattRankInfo object1, BattRankInfo object2) {
    ...
    private void addWiFiUsage(long uSecNow) {
        ...
        if (runningTimeMs < 0)
            runningTimeMs = 0;
        double wifiPower = (onTimeMs * 0 * mPowerProfile.getAveragePower(PowerProfile.POWER_WIFI_ON)
        ...
        BattRankInfo bs = addEntry(DrainType.WIFI, runningTimeMs, wifiPower + mWifiPower);
    ...
    private void addBluetoothUsage(long uSecNow) {
        ...
        BattRankInfo bs = addEntry(DrainType.BLUETOOTH, btOnTimeMs, btPower + mBluetoothPower);
    ...
    private void addRadioUsage(long uSecNow) {
        ...
        BattRankInfo bs = addEntry(DrainType.CELL, signalTimeMs, power);
        if (signalTimeMs != 0) {
            bs.noCoveragePercent = mStats.getPhoneSignalStrengthTime(0, uSecNow, mStatsType) / 1000 * 100.0
                    / signalTimeMs;
        }
    }

    private void aggregateSippers(BattRankInfo bs, List<BattRankInfo> from, String tag) {
        for (int i = 0; i < from.size(); i++) {
            BattRankInfo wbs = from.get(i);
            if (DEBUG)
           ...
           // YLog.e("bs", bs.toString());
        }
    }

    private BattRankInfo addEntry(DrainType drainType, long time, double power) {
        if (power > mMaxPower)
            mMaxPower = power;
        mTotalPower += power;
        BattRankInfo bs = new BattRankInfo(mContext, drainType, null, new double[]{power});
        bs.usageTime = time;
        mUsageList.add(bs);
        return bs;
    }
    ...
    private void processAppUsage() {
        ...
        for (int iu = 0; iu < NU; iu++) {
            ...
            if (power != 0) {
                BattRankInfo app = new BattRankInfo(mContext, DrainType.APP, u, new double[]{power});
                app.cpuTime = cpuTime;
                app.gpsTime = gpsTime;
